                ...
                <div class="container">
                    ...
                    <div class="card shadow mb-4 p-0">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            @if ($isPostTest == 1)
                                <span class="m-0 font-weight-bold text-primary text-l">Post-test</span>
                            @else
                                <span class="m-0 font-weight-bold text-primary text-l">Pre-test</span>
                            @endif
                        ...
                        <div class="card-body px-5">
                            ...
                            <div>
                                ...
                                <div class="mb-4 d-flex justify-content-between align-items-center">
                                    {{-- @if (!empty($userExercise))
                                        <div class="text-gray-700 text-l small-card">Score: <strong
                                                class="text-primary"
                                                id="exerciseScore">{{ $userExercise->score }}%</strong></div>
                                    @endif --}}

                                    @if ($userTest->pretest_taken == 1)
                                        <div class="text-gray-700 text-l small-card">Score: <strong class="text-primary"
                                                id="exerciseScore"></strong></div>
                                    @endif
                            ...
                            <p class="mb-2 mt-5 text-s">Note:
                                You must take this test to access or unlock Classic Study contents.
                            </p>
...
<script>
    console.log(@json($topicExerciseItems));

    let isPostTest = @json($isPostTest);
    ...
    async function updatePretest(score) {

        fetch('{{ route('update-pretest') }}', {
                method: 'POST',
                headers: {
    ...
    async function updatePosttest(score) {

        fetch('{{ route('update-posttest') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json'
